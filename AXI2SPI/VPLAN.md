...
主回归包含 20 个 test 名称，覆盖 24 个核心检查点。功能覆盖 `cg_spi_frame` 已达到 100%，项目自定义 SVA 覆盖率达到 100%。代码覆盖中，SPI master 主体覆盖较高，寄存器模块的条件覆盖和 toggle 覆盖仍有缺口，主要来自非法地址、未翻转响应位、保守地址译码和部分低收益分支。`ral_test` 作为独立冒烟测试保留，不计入简历。
...
| 总覆盖  | `73.82%`，受 UVM 库和低收益 RTL 分支影响               |
...
| `UVMTB/sva/axi_sva_checker.sv` | AXI stable-until-ready assert | AW/W/AR 通道在 `VALID && !READY` 后保持 `VALID` 和 payload 稳定 |
| `UVMTB/sva/axi_sva_checker.sv` | AXI response assert           | 写请求被接收后，在限定周期内返回 B 响应                                  |

项目自定义 assertion 无失败，SVA 覆盖率达到 100%。覆盖率报告中 UVM 库内部无 attempt 的 assertion 不计入项目自定义 SVA 缺口。
