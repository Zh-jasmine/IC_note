...
// 2: SCK idle high, sample on falling SCK, toggle on rising SCK
// 3: SCK idle high, sample on rising SCK,  toggle on falling SCK
        ...
        case (STATE)
            ...
            TRANSACTION: begin
                case (sck_pol_v)
                    ...
                    1: begin // SCK IDLE '1': sample/shift edges are the inverse of CPOL=0 modes.
                        case (sck_pha_v)
                            0: begin // pick up on falling edge/send on rising edge
                                if (trans_done_v) begin
                                    if (pos_sck_v) begin
                                        mosi_v    <= mosi_buff_v[bit_cnt_v-chosen_word_len_v];
                                    end
                            ...
                            1: begin // pick up on rising edge/send on falling edge
                                if (trans_done_v) begin
                                    if (pos_sck_v) begin
                                        miso_buff_v[bit_cnt_v-chosen_word_len_v] <= MISO_i;
                                    end
