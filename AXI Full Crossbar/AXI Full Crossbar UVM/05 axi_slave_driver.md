...
```text
10_uvm_work/tb/axi_agent/axi_slave_driver.svh
```

`axi_slave_driver` 的作用，和前面的 `axi_master_driver` 正好相反。master driver 是把 sequence 给出的 transaction 主动翻译成总线时序；slave memory driver 不从 sequencer 取事务，它本身就是一个被动响应模型，职责是接住 crossbar 从 M 侧过来的 `AW/W/AR`，把数据写进本地 memory，再按协议返回 `B/R`。
